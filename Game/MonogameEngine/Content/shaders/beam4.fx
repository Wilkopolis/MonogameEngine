...
float4 Strand(float2 fragCoord, float4 color, float hoffset, float hscale, float vscale, float timescale)
{
    ...
    float glow = 0.4 * uResolution.y;
    float twopi = 6.28318530718;
    float curve = 1.0 - abs(fragCoord.y - (sin(fmod(fragCoord.x * hscale / 100.0 / uResolution.x * 1000.0 + time * timescale + hoffset, twopi)) * uResolution.y * 0.1 * vscale + uResolution.y / 2.0));
    float i = 0;
...
float4 PixelShaderFunction(float2 coords: TEXCOORD0) : COLOR0
{
    ...
    if (uv.x < .1)
        fragColor *= uv.x/.1;
    if (uv.x > .25)
        fragColor *= clamp((.3 - uv.x + .7)/.7, 0, 1);
